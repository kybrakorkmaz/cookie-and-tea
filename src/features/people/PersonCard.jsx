...
const PersonCard = ({ person, isFollowing, onFollow, isSelf = false, disabled = false }) => (
    <motion.div
    ...
    >
        ...
        >
            <div className="w-14 h-14 md:w-16 md:h-16 rounded-full overflow-hidden shrink-0 border-2 border-cream shadow-inner group-hover:scale-105 transition-transform">
                <img
                    ...
                    src={person.profileImage || person.img || "/images/default-avatar.jpg"}
                    alt={person.name}
                />
            </div>
        ...
        {!isSelf && (
            <button
                ...
                className={`px-6 py-2 rounded-full font-paragraph text-sm font-bold transition-all border disabled:opacity-50 ${
                    isFollowing
